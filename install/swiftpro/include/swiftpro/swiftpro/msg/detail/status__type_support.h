// generated from rosidl_generator_c/resource/idl__type_support.h.em
// with input from swiftpro:msg/Status.idl
// generated code does not contain a copyright notice

// IWYU pragma: private, include "swiftpro/msg/status.h"


#ifndef SWIFTPRO__MSG__DETAIL__STATUS__TYPE_SUPPORT_H_
#define SWIFTPRO__MSG__DETAIL__STATUS__TYPE_SUPPORT_H_

#include "rosidl_typesupport_interface/macros.h"

#include "swiftpro/msg/rosidl_generator_c__visibility_control.h"

#ifdef __cplusplus
extern "C"
{
#endif

#include "rosidl_runtime_c/message_type_support_struct.h"

// Forward declare the get type support functions for this type.
ROSIDL_GENERATOR_C_PUBLIC_swiftpro
const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
  rosidl_typesupport_c,
  swiftpro,
  msg,
  Status
)(void);

#ifdef __cplusplus
}
#endif

#endif  // SWIFTPRO__MSG__DETAIL__STATUS__TYPE_SUPPORT_H_
